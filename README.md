...
Instalar build release (para validar R8): `./gradlew installRelease`

## ProGuard / R8
La build release utiliza `isMinifyEnabled = true` y `isShrinkResources = true`.  
Esto permite:
...
Las reglas se encuentran en `app/proguard-rules.pro`.
...
## Testing
Se incluyen **pruebas unitarias para la capa de dominio** utilizando repositorios fake para aislar la lógica de negocio.  
Las pruebas validan:
- inserción de documentos
- obtención de documentos
- eliminación de documentos

Los tests se encuentran en `app/src/test/`.

## Posibles mejoras
Para un entorno productivo podrían añadirse:
- configuración de **build flavors** (dev / staging / prod)
- **CI/CD** para automatizar builds y pruebas
- mayor cobertura de **tests unitarios e instrumentados**
- uso de **versionado semántico (SemVer)** y tags de release
- encriptación adicional del almacenamiento de archivos